...
def process():
    ...
    if not read:
        ...
        body_train = pd.read_csv("./fnc-1/tree_model/train_bodies_processed.csv", encoding='utf-8')
        stances_train = pd.read_csv("./fnc-1/tree_model/train_stances_processed.csv", encoding='utf-8')
        # training set
        train = pd.merge(stances_train, body_train, how='left', on='Body ID')
        targets = ['agree', 'disagree', 'discuss', 'unrelated']
        targets_dict = dict(list(zip(targets, list(range(len(targets))))))
        train['target'] = [targets_dict[x] for x in train['Stance']]
        # print 'train.shape:'
        # print train.shape
        ...
        test_flag = True
        if test_flag:
            body_test = pd.read_csv("./fnc-1/tree_model/test_bodies_processed.csv", encoding='utf-8')
            headline_test = pd.read_csv("./fnc-1/tree_model/test_stances_unlabeled.csv", encoding='utf-8')
        ...
        with open('./fnc-1/data.pkl', 'wb') as outfile:
            pickle.dump(data, outfile, -1)
            # print 'dataframe saved in data.pkl'

    else:
        with open('./fnc-1/data.pkl', 'rb') as infile:
